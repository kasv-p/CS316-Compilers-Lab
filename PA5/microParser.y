                 ...
                 if (node->value==":=")
                 {
                        ...
                        if (node->left->variable_type=="integer" && (node->right->variable_type=="integer"||node->right->variable_type=="int_l"))
                        {
                                if (node->left->temp==-1 && node->right->temp==-1)
                                {
                                        ...
                                        codeObject.push_back({"STOREI",node->right->variable_name,"r"+to_string(node->left->temp)});
                                        codeObject.push_back({"STOREI","r"+to_string(node->left->temp),node->left->variable_name});
                                        temporary_count++;
                                }
                                else
                                {
                        ...
                        {   
                                if (node->left->temp==-1 && node->right->temp==-1)
                                {
                                        node->left->temp=temporary_count;
                                        node->right->temp=temporary_count;
                                        codeObject.push_back({"STOREF",node->right->variable_name,"r"+to_string(node->left->temp)});
                                        codeObject.push_back({"STOREF","r"+to_string(node->left->temp),node->left->variable_name});
                                        temporary_count++;
                                }
                                else
                                {
...
pgm_body END 
{
        ...
        for (int i=0;i<codeObject.size();i++)
        {
                ...
                else if (codeObject[i][0].find("cmp")!=std::string::npos)
                {
...
cond: expr compop expr {
                ...
                temporary_count++;
                codeObject.push_back({"STORE",$3->variable_name,"r"+to_string($3->temp)});

                if ((($1->variable_type=="integer" || $1->variable_type=="int_l")&& ($3->variable_type=="integer" ||$3->variable_type=="int_l")))
                {
                      codeObject.push_back({"cmpi "+$3->variable_name+" "+"r"+to_string($3->temp)});
                }
