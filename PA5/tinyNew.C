...
public:
  ...
  float rval(){
    ...
    else if (ot==stackref) return myStack[fp-stackoffset].rval(); 
  ...
  void setFreeTime(int ft){
    ...
    case stackref:
      {
	myStack[fp-stackoffset].setFreeTime(ft);
	break;
      }
    }
  }
  int getFreeTime(){
    ...
    case stackref: {return myStack[fp-stackoffset].getFreeTime();}
    default: {return 0;}
    }
  }
  ...
  void printFreeTime(){
    ...
    case stackref: {myStack[fp-stackoffset].printFreeTime(); break; }
    }
  }
  // ^^^^^^^^^^^^^^^^^^^^^^^^ Statistics ^^^^^^^^^^^^^^^^^
  void print(){
    ...
    case empty_: {if (debug>=4) cout<<"emptyop"; break;}
...
int main(int argc, char *argv[]){
  ...
  // ^^^^^^^^^^^^^^^^^^^^^^^^ Statistics ^^^^^^^^^^^^^^^^^
	...
	myStack.push_back(StackElement(pc->o1().ival(),pc->o1().rval()));
      else
	myStack.push_back(StackElement(0,0));
      pc++;
      break;
    }
    case opcodes::pop: {
      if (pc->o1().type() != empty_) {
	pc->o1().setival(myStack.back().ival());
	pc->o1().setrval(myStack.back().rval());
      }
      myStack.pop_back();
      pc++;
      break;
    ...
    case opcodes::jsr: {
      ...
      pc++;
      myStack.push_back(StackElement(pc));
      pc = subrpc;
      break;
    }
    case opcodes::ret: {
      pc = myStack.back().aval();
      myStack.pop_back();
      break;
    }
    case opcodes::link: { 
      // push(fp); fp <- sp; reserve space on stack for local variables;
      myStack.push_back(StackElement(fp));  
      fp = myStack.size()-1;
    ...
    case opcodes::unlnk: {
      ...
      while (myStack.size()>fp+1) myStack.pop_back();
      fp = myStack.back().fval();
      myStack.pop_back();
      pc++;
      break;
    }
